...
namespace WindowsFormsApp1
{
    public partial class MullerForm : UserControl
    {
        ...
        private void InitializeComponent()
        {
            ...
            var lblApproximations = new Label();
            lblApproximations.Text = "Aproximaciones iniciales (x0, x1, x2):";
            lblApproximations.Font = new Font("Segoe UI", 11, FontStyle.Bold);
            lblApproximations.AutoSize = true;
            lblApproximations.Location = new Point(0, yPos);
            mainPanel.Controls.Add(lblApproximations);
            ...
            txtX1.Width = 100;
            txtX1.Height = 30;
            txtX1.Location = new Point(110, yPos + 30);
            txtX1.Text = "2";
            ...
            lblX1Label.Location = new Point(110, yPos + 65);
            mainPanel.Controls.Add(lblX1Label);

            var txtX2 = new TextBox();
            txtX2.Name = "txtX2";
            txtX2.Width = 100;
            txtX2.Height = 30;
            txtX2.Location = new Point(220, yPos + 30);
            txtX2.Text = "2.5";
            mainPanel.Controls.Add(txtX2);
            ...
            lblX2Label.Location = new Point(220, yPos + 65);
            mainPanel.Controls.Add(lblX2Label);
            ...
            btnCalculate.Click += (s, e) => CalculateMuller(mainPanel, txtFunction, txtX0, txtX1, txtX2, txtTolerance, txtMaxIterations);
        ...
        private void CalculateMuller(Panel mainPanel, TextBox txtFunction, TextBox txtX0, TextBox txtX1, TextBox txtX2, TextBox txtTolerance, TextBox txtMaxIterations)
        {
            try
            {
                ...
                while (error > tolerance && iteration < maxIter)
                {
                    ...
                    double fx2 = MathExpressionEvaluator.Evaluate(function, x2);

                    // Diferencias divididas
                    double h0 = x1 - x0;
                    double h1 = x2 - x1;
                    double delta0 = (fx1 - fx0) / h0;
                    double delta1 = (fx2 - fx1) / h1;
                    ...
                    if (Math.Abs(a) < 1e-10)
                    {
                        MessageBox.Show("Coeficiente 'a' es muy cercano a cero. No se puede continuar.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }

                    double denominator = (Math.Abs(b + Math.Sqrt(discriminant)) > Math.Abs(b - Math.Sqrt(discriminant))) ?
                        (b + Math.Sqrt(discriminant)) : (b - Math.Sqrt(discriminant));

                    double x3 = x2 - (2 * c) / denominator;
                    error = Math.Abs(x3 - x2);

                    dgv.Rows.Add(iteration + 1, Math.Round(x0, 6), Math.Round(x1, 6), Math.Round(x2, 6), Math.Round(x3, 6), Math.Round(error, 6));
